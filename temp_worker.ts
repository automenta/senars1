...
self.onmessage = async (event: MessageEvent) => {
    ...
    switch (type) {
        ...
        case 'process':
            const { task } = payload;
            try {
                const { derivedTasks, newAtoms, processedTask } = await processTask(task);
                self.postMessage({
                    type: 'result',
                    payload: {
                        derivedTasks,
...
async function processTask(task_a: Task): Promise<{ derivedTasks: Task[], newAtoms: SemanticAtom[], processedTask?: Task }> {
    ...
    // If the original task was a belief, it has been processed and should be returned for memorization.
    const processedTask = task_a.type === 'BELIEF' ? task_a : undefined;

    return { derivedTasks: allDerivedTasks, newAtoms, processedTask };
}
